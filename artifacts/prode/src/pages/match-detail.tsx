...
export default function MatchDetail() {
  ...
  const { data: match, isLoading } = useGetMatch(id, {
    query: { refetchInterval: 60000, enabled: !!id }
  });
  ...
  const canEdit = isBefore(new Date(), match.matchDate) && match.status === "scheduled";
      ...
      <div className="p-6 md:p-10 max-w-4xl mx-auto w-full space-y-8">
        ...
        <Card className="p-8 bg-card border-border max-w-2xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-2xl font-display font-bold">Your Prediction</h3>
          </div>
          ...
          {match.myPrediction?.points != null && isFinished && (
            <div className="mb-6 p-4 bg-accent/10 border border-accent/20 rounded-lg flex justify-between items-center">
              <span className="font-semibold text-accent-foreground">Points earned:</span>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              {canEdit ? (
                <div className="flex justify-center mt-8">
                  ...
                  >
                    {upsertPrediction.isPending ? "Saving..." : "Save Prediction"}
                  </Button>
                </div>
              ) : (
                <div className="text-center mt-6 text-muted-foreground bg-muted p-4 rounded-lg">
                  {isFinished ? "Match has ended. Predictions are locked." : "Predictions are locked. Match has started."}
                </div>
              )}
            </form>
          </Form>
